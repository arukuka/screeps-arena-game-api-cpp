...
ライブラリ自体に手を入れるときの手引き。
ボットを書くだけなら [README](../README.ja.md) と [`template/`](../template/) で足りる。
...
オブジェクト配列を返す API を足すときは、境界越えのコストに注意すること。
現在の `emscripten::val` 方式のトレードオフと、スナップショット方式へ
切り替える場合の判断材料は [DESIGN.ja.md](DESIGN.ja.md) にまとめてある。
