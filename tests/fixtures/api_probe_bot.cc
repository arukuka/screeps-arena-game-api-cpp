...
void check(const std::string& name, bool condition, const std::string& details = "") {
    ++g_total_assertions;
    if (condition) {
        ++g_passed_assertions;
        std::cout << "  [PASS] " << name;
...
void testPhase1_EnvironmentAndMetadata() {
    ...
    check("arenaInfo().ticksLimit", info.ticksLimit > 0, "ticksLimit=" + std::to_string(info.ticksLimit));
    check("arenaInfo().cpuTimeLimit", info.cpuTimeLimit > 0, "cpuTimeLimit=" + std::to_string(info.cpuTimeLimit));
    check("arenaInfo().cpuTimeLimitFirstTick", info.cpuTimeLimitFirstTick > 0, "firstTickLimit=" + std::to_string(info.cpuTimeLimitFirstTick));

    int t_origin = getTerrainAt(Position{0, 0});
    int t_center = getTerrainAt(Position{50, 50});
    check("getTerrainAt(0,0)", t_origin == TERRAIN_WALL, "t=" + std::to_string(t_origin));
    check("getTerrainAt(50,50)", t_center == TERRAIN_PLAIN || t_center == TERRAIN_SWAMP || t_center == TERRAIN_WALL,
    ...
    check("getDirection()", d_top == TOP && d_right == RIGHT && d_bottom == BOTTOM && d_left == LEFT,
          "TOP=" + std::to_string(d_top) + " RIGHT=" + std::to_string(d_right));

    const auto& obstacles = obstacleObjectTypes();
    check("obstacleObjectTypes()", !obstacles.empty(), "count=" + std::to_string(obstacles.size()));

    const auto& resources = resourcesAll();
    check("resourcesAll()", !resources.empty(), "count=" + std::to_string(resources.size()));
...
void testPhase2_PathFinderAndVisuals() {
    ...
    SearchPathResult sp_res = searchPath(Position{10, 10}, Goal{Position{30, 30}, 1}, sp_opts);
    check("searchPath(single goal)", !sp_res.incomplete, "ops=" + std::to_string(sp_res.ops) + " path.len=" + std::to_string(sp_res.path.size()));

    std::vector<Goal> multi_goals = { Goal{Position{20, 20}, 0}, Goal{Position{40, 40}, 0} };
    SearchPathResult multi_sp_res = searchPath(Position{10, 10}, multi_goals, sp_opts);
    ...
    fp_opts.swampCost = 5;
    fp_opts.maxOps = 500;
    std::vector<Position> fp_path = findPath(Position{15, 15}, Position{25, 25}, fp_opts);
    check("findPath()", !fp_path.empty(), "path.len=" + std::to_string(fp_path.size()));

    Visual vis(1, false);
    check("Visual::layer()", vis.layer() == 1);
    check("Visual::persistent()", !vis.persistent());

    vis.circle(Position{50, 50}, CircleStyle{.radius = 2.0, .fill = "#ff0000", .opacity = 0.5});
    vis.line(Position{10, 10}, Position{20, 20}, LineStyle{.width = 0.2, .color = "#00ff00"});
    vis.rect(Position{30, 30}, 5, 5, RectStyle{.fill = "#0000ff"});
    vis.poly({Position{60, 60}, Position{65, 60}, Position{63, 65}}, PolyStyle{.fill = "#ffff00"});
    vis.text("PROBE", Position{50, 48}, TextStyle{.align = "center", .color = "#ffffff"});

    check("Visual::size()", vis.size() > 0, "bytes=" + std::to_string(vis.size()));
    vis.clear();
...
void testPhase3_PrototypesAndSnapshots() {
    ...
    check("getObjectsByPrototype<Creep>()", !creeps.empty(), "creeps=" + std::to_string(creeps.size()));
    check("getObjectsByPrototype<Flag>()", !flags.empty(), "flags=" + std::to_string(flags.size()));
    check("Empty prototypes return safely",
          spawns.empty() && extensions.empty() && towers.empty() && containers.empty() &&
          ramparts.empty() && roads.empty() && walls.empty() && sources.empty() &&
          resources.empty() && sites.empty());

    if (!creeps.empty()) {
        const auto& c = creeps[0];
        check("Creep::record()", c.record() >= 0);
        check("Creep::x(), y(), pos()", c.x() >= 0 && c.y() >= 0 && c.pos().x == c.x());
...
void testPhase4_ObjectDetailsAndStore() {
    ...
    if (c_opt) {
        ...
        check("Handle reads match snapshot", c.hits() == c_proto.hits() && c.x() == c_proto.x());

        auto body = c.body();
        check("Creep::body()", !body.empty(), "parts=" + std::to_string(body.size()));
        ...
        check("GameObject::effects()", true, "count=" + std::to_string(effects.size()));
        check("GameObject::ticksToDecay()", true);
        check("GameObject::getRangeTo()", c.getRangeTo(Position{50, 50}) >= 0);
    }

    auto site_result = createConstructionSite(Position{50, 50}, StructureWall::kPrototype);
    check("createConstructionSite()", true, "success=" + std::to_string((bool)site_result));
...
void testPhase5_ActionIntents() {
    ...
    for (const auto& c : creeps) {
        if (c.my()) my_creeps.push_back(c);
        else foe_creeps.push_back(c);
    }

    check("My creeps available", !my_creeps.empty(), "my=" + std::to_string(my_creeps.size()));
    ...
    if (!my_creeps.empty()) {
        ...
        check("creep.move(direction)", res_move == OK || res_move == ERR_TIRED || res_move == ERR_NO_BODYPART);

        int res_moveTo = c.moveTo(Position{50, 50});
        check("creep.moveTo(pos)", res_moveTo == OK || res_moveTo == ERR_TIRED || res_moveTo == ERR_NO_BODYPART);

        int res_attack = c.attack(dummy_target);
        check("creep.attack(target)", res_attack == OK || res_attack == ERR_NOT_IN_RANGE || res_attack == ERR_NO_BODYPART);

        int res_r_attack = c.rangedAttack(dummy_target);
        check("creep.rangedAttack(target)", res_r_attack == OK || res_r_attack == ERR_NOT_IN_RANGE || res_r_attack == ERR_NO_BODYPART);

        int res_mass = c.rangedMassAttack();
        check("creep.rangedMassAttack()", res_mass == OK || res_mass == ERR_NO_BODYPART);

        int res_heal = c.heal(heal_target);
        check("creep.heal(target)", res_heal == OK || res_heal == ERR_NOT_IN_RANGE || res_heal == ERR_NO_BODYPART);

        int res_r_heal = c.rangedHeal(heal_target);
        check("creep.rangedHeal(target)", res_r_heal == OK || res_r_heal == ERR_NOT_IN_RANGE || res_r_heal == ERR_NO_BODYPART);

        if (my_creeps.size() >= 2) {
            int res_pull = c.pull(my_creeps[1]);
            check("creep.pull(target)", res_pull == OK || res_pull == ERR_NOT_IN_RANGE || res_pull == ERR_INVALID_TARGET);
        }

        int res_drop = c.drop(RESOURCE_ENERGY, 1);
        check("creep.drop(resource, amount)", res_drop == OK || res_drop == ERR_NOT_ENOUGH_RESOURCES);

        auto resources = getObjectsByPrototype<Resource>();
        if (!resources.empty()) {
            int res_pickup = c.pickup(resources[0]);
            check("creep.pickup(resource)", res_pickup == OK || res_pickup == ERR_NOT_IN_RANGE);
        }

        int res_transfer = c.transfer(heal_target, RESOURCE_ENERGY, 1);
        check("creep.transfer(target, resource)", res_transfer == OK || res_transfer == ERR_NOT_IN_RANGE || res_transfer == ERR_NOT_ENOUGH_RESOURCES);

        auto spawns = getObjectsByPrototype<StructureSpawn>();
        if (!spawns.empty()) {
            int res_withdraw = c.withdraw(spawns[0], RESOURCE_ENERGY, 1);
            check("creep.withdraw(structure)", res_withdraw == OK || res_withdraw == ERR_NOT_IN_RANGE);
        }

        auto sources = getObjectsByPrototype<Source>();
        if (!sources.empty()) {
            int res_harvest = c.harvest(sources[0]);
            check("creep.harvest(source)", res_harvest == OK || res_harvest == ERR_NOT_IN_RANGE || res_harvest == ERR_NO_BODYPART);
        }

        auto sites = getObjectsByPrototype<ConstructionSite>();
        if (!sites.empty()) {
            int res_build = c.build(sites[0]);
            check("creep.build(site)", res_build == OK || res_build == ERR_NOT_IN_RANGE || res_build == ERR_NO_BODYPART);
        }
    }
}
